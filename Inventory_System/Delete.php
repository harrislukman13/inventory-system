    ...
    <div class="wrapper">
        ...
        <div id="content">
            ...
            if ( isset($_POST['delete']) && isset($_POST['item_id']) ) {
                $sql = "DELETE FROM inventory WHERE item_id = :zip";
            ...
            // Guardian: Make sure that item_id is present
            if ( ! isset($_GET['item_id']) ) {
              $_SESSION['error'] = "Missing item_id";
              header('Location: index.php');
              return;
            }

            $stmt = $pdo->prepare("SELECT item_name, item_id FROM inventory where item_id = :xyz");
            ...
            ?>
            <center><p>Confirm: Deleting <?= htmlentities($row['item_name']) ?></p></center>
            ...
            if ( isset($_POST['item_id']) && isset($_POST['item_name'])
                ...
                $sql = "INSERT INTO inventory(item_id,item_name,quantity,price) VALUES (:item_id, :item_name, :quantity,:price)";
                $stmt = $pdo->prepare($sql);
                $stmt->execute(array(
