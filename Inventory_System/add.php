...
if( isset($_POST['unit_price']) && isset($_POST['item_name']) && isset($_POST['qtty'] ) ) {
  ...
  if( strlen($_POST['unit_price']) < 1 && strlen($_POST['item_name']) < 1 && strlen($_POST['qtty']) < 1)
    $_SESSION['message'] = "Fill in the blanks to add";
  else {
 ...
 $_SESSION['qtty'] = htmlentities($_POST['qtty']);
  ...
  #  echo nl2br($_SESSION['unit_price']."\n");
     ...
     if ( is_numeric($_SESSION['qtty']) && is_numeric($_SESSION['unit_price']) ) {
      $sql= "Insert into inventory (Item_name,Quantity,Price) values (:it,:qt,
            :up ) ;";
...
<h1 class="thickblack"> Add Inventory </h1><br><br><br>

<?php
if (strlen($_SESSION['message']) > 0 ) {
if (strpos($_SESSION['message'],'inserted') === false ) {
...
<div>
  ...
  <div class="form-group">
    <div class="form-row" >
    <label class="control-label col-sm-2"  for="qtty"> Quantity:</label>
  ...
  <div class="form-group">
    <div class="form-row" >
    <label class="control-label col-sm-2"  for="unit_price"> Unit Price (RM):</label>
  ...
  <div class="form-group">
    ...
    <div class="col-sm-offset-2 col-sm-10">
      <a id="a" href="https://www.javatpoint.com/html-image-button" ><svg xmlns="http://www.w3.org/2000/svg" width="40" height="40" fill="currentColor" class="bi bi-x-circle" viewBox="0 0 16 16">
      ...
      <input type="submit" class=" button btn btn-outline-secondary btn-lg"  value="Add"/>
    </div>
  </div>
  </form>
